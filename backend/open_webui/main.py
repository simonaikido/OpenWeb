...
async def get_available_webhook_events(user=Depends(get_verified_user)):
    from open_webui.utils.webhook_events import (
        get_available_events,
        get_event_categories,
    )

    return {"events": get_available_events(), "categories": get_event_categories()}
...
async def create_webhook_config(form_data: dict, user=Depends(get_verified_user)):
    ...
    if user.role != "admin":
        ...
        user_id = form_data[
            "user_id"
        ]  # Admins can specify user_id or leave None for global
    ...
    if result:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to create webhook configuration"
        )
...
async def update_webhook_config(
    webhook_id: str, form_data: dict, user=Depends(get_verified_user)
):
    ...
    if result:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to update webhook configuration"
        )
...
async def delete_webhook_config(webhook_id: str, user=Depends(get_verified_user)):
    ...
    if success:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to delete webhook configuration"
        )
